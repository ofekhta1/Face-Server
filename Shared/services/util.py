    ...
    for models in generated_embeddings:
        ...
        if f"{return_detector}_{embedder}" not in generated_embeddings:
            if return_detector not in model_loader.model_registry["detectors"]:
                raise Exception(
                    f"The return_detector {return_detector} does not exist!"
                )
            else:
                return BaseError(
                    reason=f"No embeddings were created with return detector and the embedder {embedder}"
                )
        if detector != return_detector:
            base_detector_embs = generated_embeddings[f"{return_detector}_{embedder}"]
            other_embeddings = generated_embeddings[models]
            if len(other_embeddings) > 0:
                similarity_matrix = cosine_similarity(
                    base_detector_embs, other_embeddings
                )
                print("Similarity Matrix:")
                print(similarity_matrix)
                detector_indices[detector] = convert_detector_indices(similarity_matrix)

        detector_metadata[detector] = image_metadata.get_image_faces(
            filename, -2, detector, embedder
        )
...
def convert_detector_indices(similarity_matrix, tolerance=0.1):
    ...
    for col_index, column in enumerate(
        similarity_matrix.T
    ):  # Transpose the array to iterate over columns
...
def transform_norm_landmarks(landmarks: np.ndarray):
    ...
    landmarks_homo = np.hstack(
        [landmarks, np.ones((landmarks.shape[0], 1))]
