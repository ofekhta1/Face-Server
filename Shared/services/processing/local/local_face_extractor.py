...
from Shared.services.models.face_embedding.quality.local.resnet_quality import BaseQualityModel
class LocalFaceExtractor:
    ...
    def extract_faces(self,filename: str, model: BaseDetectorModel,quality_type="face_size_quality",faces_dir="")->tuple[cv2.typing.MatLike,list]:
        img =MediaLoader.load_image(filename,model)
        if img is not None:
            faces= model.extract_faces(img)
            quality_model=self.model_loader.load_quality(quality_type)
            faces=self.add_quality_to_faces(img,faces,model=quality_model);
            return img,faces;
        return None,None
    
    def extract_faces(self,img, model: BaseDetectorModel,quality_type="face_size_quality",faces_dir="")->tuple[cv2.typing.MatLike,list]:
        if img is not None:
            faces= model.extract_faces(img)
            quality_model=self.model_loader.load_quality(quality_type)
            faces,_=self.add_quality_to_faces(img,faces,model=quality_model);
            return img,faces;
        return img,None