...
from Shared.services.models.face_embedding.genderage.base_genderage_model import BaseGenderAgeModel
import numpy as np
...
class TritonEmbeddingGenerator:

    def __init__(self,emb_manager:BaseImageEmbeddingManager,face_extractor:TritonFaceExtractor,image_storage:BaseImageStorage) :
    ...
    def generate_all_emb(
        ...
        for i in range(len(faces)):
            try:
                embedding = embedder.embed(img, faces[i]).flatten()
                ...
                return FaceEmbeddingError(embedder_name=embedder.name,reason=f"Failed to generate face embedding for aligned_{i}_{filename}: {ex}")
