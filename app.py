                                ...
                                if len(temp_err) > 0 or (not faces):
                                    ...
                                    if save_invalid:
                                        os.replace(
                                            path,
                                            os.path.join(
                                                UPLOAD_FOLDER, "no_face", file.filename
                                            ),
                                        )
                                    else:
                                        os.remove(path)
    ...
    if most_similar_image:
        ...
        face_length = len(
            helper.get_face_boxes(
                most_similar_image.image_name, detector_name, embedder_name
            )
        )
        ...
        for detector in ModelLoader.detectors:
            for embedder in ModelLoader.embedders:
                generated_embeddings[f"{detector}_{embedder}"] = (
                    helper.emb_manager.get_image_embeddings(
                        image_name, detector, embedder
                    )
                )
        detector_indices = util.get_all_detectors_faces(
            generated_embeddings, detector_name
        )
        return jsonify(
            {
                "image": image_name,
