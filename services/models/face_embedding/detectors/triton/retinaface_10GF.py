...
class TritonRetinaFace10GF(BaseDetectorModel):
    ...
    def __detect_with_size(self,img,input_size):
        blob,det_scale = self.detection_preprocessing(img, input_size=input_size) 
        detection_input = httpclient.InferInput("input.1", blob.shape, datatype="FP32")
        detection_input.set_data_from_numpy(blob, binary_data=True)
        detection_response=TritonClientHandler.infer(model_name="retinaface_detector",inputs=[detection_input])
    ...
    def extract_faces(self,img)->list[dict]:
        start=time.time()
        far_results=self.__detect_with_size(img,(1024,1024))
        close_results=self.__detect_with_size(img,(320,320))
